...
const CBTExercise5 = () => {
  ...
  const interactiveSteps = [1, 2, 3, 4, 5, 6, 7];
  ...
  const isReadyToComplete = () => {
    // Ensure all required responses (responses[0]..responses[6]) are present
    return (
      ...
      responses[5]?.trim().length > 0 &&
      responses[6]?.trim().length > 0
    );
  };
